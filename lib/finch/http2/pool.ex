defmodule Finch.HTTP2.Pool do
  ...
  # Call the pool with teh request. The pool will multiplex multiple requests
  ...
  def request(pool, request, opts) do
    ...
    with {:ok, ref} <- :gen_statem.call(pool, {:request, request, opts}) do
      ...
      response_waiting_loop(ref, monitor, %Response{}, fail_safe_timeout)
    end
  end
  ...
  def disconnected(:enter, _, data) do
    :ok = Enum.each(data.requests, fn {from, ref} ->
      send(from, {:error, ref, "connection closed"})
    end)
  ...
  def disconnected({:timeout, :reconnect}, failure_count, data) do
 ...
 def disconnected({:timeout, {:request_timeout, _ref}}, _content, _data) do
  ...
  def connected_read_only({:call, from}, {:request, _, _}, _) do
    {:keep_state_and_data, {:reply, from, {:error, "read only"}}}
  end
